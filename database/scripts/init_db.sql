...
CREATE INDEX rol_data on roles(role_name);
...
CREATE INDEX users_data on users(full_name, document_number);
...
CREATE INDEX shop_data on shops(shop_address);
...
CREATE INDEX product_data on products( name_product, price, stock);
...
CREATE INDEX user_orders_data on user_orders(order_date);
...
CREATE INDEX order_details_data on order_details(amount, unit_price);  
-- Table: payments
...
CREATE INDEX payments_data on payments(payment_amount, payment_date);
...
CREATE INDEX shipments_data on shipments (transporter);
